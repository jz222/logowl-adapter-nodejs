...
const init = (conf) => {
    ...
    if (isValid) {
        ...
        console.error(constants.colors.red + 'The provided Log Owl configuration is invalid.' + constants.colors.reset);
    }
};

const emitError = (error, onTheFlyBadges = {}) => {
    if (!error || !(error instanceof Error)) {
        console.error('[LOG OWL] the provided error must be an instance of Error');
        return;
    }
    
    if (typeof onTheFlyBadges !== 'object' || Array.isArray(onTheFlyBadges)) {
        console.error('[LOG OWL] on the fly badges must be an object');
        return;
    }
    
    if (Object.keys(onTheFlyBadges || {}).some(b => typeof onTheFlyBadges[b] !== 'string')) {
        console.error('[LOG OWL] on the fly badges need to be an object of strings');
        return;
    }
